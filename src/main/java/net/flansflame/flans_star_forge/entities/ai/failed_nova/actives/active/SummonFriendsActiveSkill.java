...
public class SummonFriendsActiveSkill extends FailedNovaActiveSkill {

    public static final int FRIENDS_DETECT_RADIUS = 50;
    public static final int MAX_FRIEND = 12;
    public static final int MAX_LOOPS = 20;

    public SummonFriendsActiveSkill(String animationId, SoundEvent attackSound, boolean activateEvenIfNotNear) {
        ...
        if (nova.level() instanceof ServerLevel server) {
            ...
            if (followerCount < MAX_FRIEND) {
                for (int i = 0; i < 3; i++) {
                    ...
                    var entityToSpawn = ModEntities.FRIEND.get().spawn(server, BlockPos.containing(vX, y, vZ), MobSpawnType.MOB_SUMMONED);
            ...
            } else {
                {
                    ...
                    nova.addEffect(new MobEffectInstance(MobEffects.INVISIBILITY, 40, 20));
                    nova.addEffect(new MobEffectInstance(MobEffects.GLOWING, 40, 20));
                }
            }
        }
        super.onAttack(nova, target, amount);
    }
}
